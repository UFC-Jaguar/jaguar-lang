...
namespace FrontEnd.Grammar {
    public class IfElse : Grammar {
        ...
        public IfElse(List<NoIF.NoDataIFs> buffer) { 
        ...
        public AstInfo Rule(Parser parser) {
            ...
            if (ast.Error != null)
                return ast;
            return ast.Success(new NoIF(this.BufferNoIF, ((NoIF)noElse).Else));
        }
        private AstInfo Call_Else(Parser parser) {
            ...
            if (ast.Error != null)
                return ast;
            noElse = NoIF.ElseInstance(statements);
            ...
            parser.NextToken(ast);
            //}
            return ast.Success(new NoIF(this.BufferNoIF, noElse));
        }
    }
}
